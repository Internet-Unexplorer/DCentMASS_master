...
if(isset($_POST["uname"])){
	...
	$compny = preg_replace('#[^a-zA-Z. ]#i', '', $_POST['compny']);
	...
	if($uname == "" || $dname == "" ||  $emai == "" ||  $pas1 == "" || $pas2 == "" || $loc == "" || $compny == ""){
	...
	// END FORM DATA ERROR HANDLING
		...
		$sql = "INSERT INTO `employer` (`userName`, `displayName`,`email`, `location`, `companyName`) 
		VALUES ('$uname', '$dname', '$emai', '$loc', '$compny')";
		$query = mysqli_query($connect_db, $sql);
		$uid = mysqli_insert_id($connect_db);


		$sql = "INSERT INTO `login_info` (`userName`, `email`, `password`, `signe_up_dateTime`, `loginDateTime`, `active`, `userType`, `ip`) 
		VALUES ('$uname', '$emai', '$p_hash', now(), now(), '0', 'employer', '$ip');";
		$query = mysqli_query($connect_db, $sql);
		$uid = mysqli_insert_id($connect_db);
...
<form name="signupform" id="signupform" onSubmit="return false;" style="padding:20px;">
  <h2 style="margin-left:5px;">Signingup as Employer</h2><i>All the fields are required.</i>
  <br>
  
  <div class="input-group input-group-lg">
    <input id="username" type="text" onBlur="checkuname()" onFocus="emptyElement('status')" onKeyUp="restrict('username')" maxlength="40" class="form-control" placeholder=" Enter Username" style="border-radius: 0px; margin:5px;">
   </div>
   ...
   <div class="input-group input-group-lg">
    ...
    <div class="input-group input-group-lg">
    <input class="form-control" placeholder="Enter Company Name" id="company" type="text" onFocus="emptyElement('status')" maxlength="100" style="border-radius: 0px; margin:5px;">
    </div>
...
function checkemail(){
	...
	if(u != ""){
		...
		var ajax = ajaxObj("POST", "<?php echo $siteAddress; ?>/employer.php");
...
function signup(){
	...
	_("company").style.border = "1px solid #ccc";
    ...
    if (compny == "") {
		_("company").style.border = "2px solid #f60";
	}
	if (pas1 != pas2){
	...
	if(uname == "" || dname == "" || emai == "" || pas1 == "" || pas2 == "" || loc == "" || compny == "" ){
        ...
        ajax.send("uname="+uname+"&dname="+dname+"&emai="+emai+"&pas1="+pas1+"&pas2="+pas2+"&loc="+loc+"&compny="+compny);
	}
}
    </script>
</body>
